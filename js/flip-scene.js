...
(function () {
  ...
  function getStartPos() {
    var sx = localStorage.getItem('icon1StartX');
    var sy = localStorage.getItem('icon1StartY');
    /* Saved values (from the dev tool) are kept as-is. The fallback defaults
       scale with the viewport — anchored to 1920×1080 — so the fly-in starts
       from a proportional position on every screen size. */
    return {
      x: sx !== null ? parseFloat(sx) : -520 * (window.innerWidth  / 1920),
      y: sy !== null ? parseFloat(sy) :  280 * (window.innerHeight / 1080)
    };
  }
